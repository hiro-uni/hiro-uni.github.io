...
記事内やページ内で外部サイトのリンクをリッチなブログカード形式で表示する方法です。

### 1. リンクの記述方法

Markdown + JSX（.mdx）や Astro ファイル内で、以下の形式で記述します。

```astro
<OGP url="https://example.com/article" />
```

※ ファイルで使用する場合は、コンポーネントのインポートが必要です。
```markdown
...
新しいリンクを追加した際や、情報を更新したい場合は、以下のコマンドを実行してキャッシュを更新します。

```sh
npm run fetch-ogp
```

このコマンドを実行すると、プロジェクト内のファイルからURLを抽出し、タイトルや画像を自動的に取得して `src/data/ogp-cache.json` に保存します。ビルド時にこのキャッシュを参照してカードを表示します。
